...
public:
    ...
    removePlayer(Player *p)
    {
        ...
        if(iter!=players.end())
        { std::deque<std::pair<Session*,EncapsulatedPacket>>::iterator it= PacketsQueue.begin();

             while(it!=PacketsQueue.end()){
                if(it->first==(*iter)->connection)
                    it=PacketsQueue.erase(it);
                else
                it++;
